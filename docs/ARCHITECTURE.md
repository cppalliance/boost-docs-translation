...
| Setup-phase helpers (**`resolve_add_submodules_names`**, **`ensure_all_translation_lang_branches`**, **`ensure_translations_cloned`** failures) | Return **`1`** and cause **immediate job `exit`** before batch processing |
